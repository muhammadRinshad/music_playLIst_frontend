...
export default function PlaylistDetails() {
    ...
    return (
      <div className="p-4 sm:p-6 max-w-4xl mx-auto w-full min-w-0">
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 mb-6 sm:mb-8">
          <div className="w-full sm:w-40 md:w-48 aspect-square sm:aspect-auto sm:h-40 md:h-48 rounded-xl sm:rounded-2xl bg-zinc-800 animate-pulse shrink-0" />
          <div className="flex-1 min-w-0">
            <div className="h-4 bg-zinc-800 rounded w-20 mb-3" />
            <div className="h-8 sm:h-10 bg-zinc-800 rounded w-3/4 sm:w-48 mb-2" />
            <div className="h-4 bg-zinc-800 rounded w-24" />
          </div>
        </div>
        <div className="space-y-2">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="h-14 sm:h-16 bg-zinc-900/50 rounded-xl animate-pulse" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="p-4 sm:p-5 md:p-6 max-w-4xl mx-auto w-full min-w-0">
      {/* Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-end gap-4 sm:gap-6 mb-6 sm:mb-10 animate-fadeSlideUp">
        <div className="w-full sm:w-40 md:w-48 aspect-square sm:aspect-auto sm:h-40 md:h-48 rounded-xl sm:rounded-2xl bg-gradient-to-br from-emerald-900/50 to-zinc-800 flex items-center justify-center text-5xl sm:text-6xl md:text-7xl shadow-xl shrink-0">
          ♪
        </div>
                    ...
                    <Draggable key={song._id} draggableId={song._id} index={idx}>
                      {(provided, snapshot) => (
                        <div
                          ...
                          className={`group flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-xl transition ${
                            snapshot.isDragging
                              ? "bg-zinc-700/80 shadow-lg"
                              : "hover:bg-zinc-200 dark:hover:bg-zinc-800/60"
                          }`}
                        >
                          <div
                            {...provided.dragHandleProps}
                            className="cursor-grab active:cursor-grabbing text-zinc-400 p-1 shrink-0 touch-none"
                          >
                            <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="currentColor" viewBox="0 0 24 24">
                              <path d="M8 6h2v2H8V6zm0 5h2v2H8v-2zm0 5h2v2H8v-2zm5-10h2v2h-2V6zm0 5h2v2h-2v-2zm0 5h2v2h-2v-2z" />
                            </svg>
                          </div>
                          <span className="text-zinc-500 text-xs sm:text-sm w-5 sm:w-6 tabular-nums shrink-0">
                            {idx + 1}
                          </span>
                          <SongCard
                            song={song}
      ...
      {/* Add songs */}
      <section>
        <h2 className="text-base sm:text-lg font-semibold mb-3 sm:mb-4">Add songs</h2>
          ...
          <div className="space-y-1">
            {availableSongs.map((song) => (
              <div
                key={song._id}
                onClick={() => play(song)}
                className="group flex items-center gap-3 sm:gap-4 p-2 sm:p-3 rounded-xl hover:bg-zinc-800/60 transition cursor-pointer"
              >
                <div className="flex-1 min-w-0">
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); addSong(song._id); }}
                  className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg bg-emerald-500/20 text-emerald-400 font-medium text-xs sm:text-sm hover:bg-emerald-500/30 transition shrink-0 active:scale-95"
                >
                  Add
                </button>
              </div>
